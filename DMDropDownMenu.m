...
- (void)p_setUpView
{
    ...
    _curText.font = [UIFont systemFontOfSize:13];
...
- (void)tapAction
{
    ...
    }else
    {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            if ( _listArr.count*30 > [UIScreen mainScreen].bounds.size.height/2) {
                
                frame.size.height = [UIScreen mainScreen].bounds.size.height/2;
            }else{
                frame.size.height = _listArr.count*30;
            }
            [_menuTableView setFrame:frame];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    label.text = _listArr[indexPath.row];
...
{
    [self tapAction];
    _curText.text = _listArr[indexPath.row];
...
{
    NSString * string = _listArr[indexPath.row];
    CGRect bounds = [string boundingRectWithSize:CGSizeMake(self.frame.size.width - 40, 200) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:11] } context:NULL];
