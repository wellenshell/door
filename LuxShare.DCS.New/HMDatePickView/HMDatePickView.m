...
- (void)configuration {
    //时间选择器
    UIView *dateBgView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 130, [UIScreen mainScreen].bounds.size.width, 200)];
    dateBgView.backgroundColor = [UIColor whiteColor];
    [self addSubview:dateBgView];
    
    //确定按钮
    UIButton *commitBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    commitBtn.frame = CGRectMake(dateBgView.bounds.size.width - 70, 0, 60, 40);
    commitBtn.tag = 1;
...
- (void)pressentPickerView:(UIButton *)button {
    ...
    if (button.tag == 1) {
        //确定
        if ([self.hmDatePicker.date compare:[NSDate date]] == NSOrderedAscending) {
              [KPromptBox showKPromptBoxWithMassage:@"不能小于当前时间"];
        }else{
