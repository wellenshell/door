...
-(void)creatView{
    ...
    NSArray *titleArr = @[@"工        号:",@"姓        名:",@"性        别:",@"部        门:",@"职位名称:",@"资       产:",@"携带资产出厂:"];
    
    for (int i =0; i<7; i++) {
        
        CGSize size = [titleArr[i] sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:[UIFONT fontWithDevice:14]],NSFontAttributeName, nil]];
        
        CGRect rect = [detailArr[i] boundingRectWithSize:CGSizeMake(self.view.frame.size.width-size.width-55-8, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:[UIFONT fontWithDevice:14]],NSFontAttributeName, nil] context:nil];
        
        UILabel *labDetail = [[UILabel alloc]init];
        labDetail.textAlignment = NSTextAlignmentLeft;
        [labDetail setNumberOfLines:0];
        labDetail.tag = i+20;
        labDetail.font = [UIFont systemFontOfSize:[UIFONT fontWithDevice:14]];
        ...
        if (i == 6) {
            ...
            labDetail.sd_layout
            .leftSpaceToView(self.view,SCREENWIDTH/7+size.width+5)
            .rightSpaceToView(self.view,0)
            .heightIs(rect.size.height)
            .topSpaceToView(laa,20);
        }else{
            if (i == 5) {
                ...
                .rightSpaceToView(self.view,0)
                .heightIs(rect.size.height)
                .topSpaceToView(pic,40+(i%7)*(rect.size.height+20));
            }
        }
        ...
        labName.font = [UIFont systemFontOfSize:[UIFONT fontWithDevice:14]];
        [self.view addSubview:labName];
        if (i == 6) {
            labName.sd_layout
            .leftSpaceToView(self.view,SCREENWIDTH/7)
